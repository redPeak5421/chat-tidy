...
test('support cards open official pages only on click in persistent tabs',async()=>{
 ...
 try{
 w.chrome={storage:{local:{get:async d=>d,set:async()=>{}}},runtime:{getURL:path=>'https://extension.test/'+path},tabs:{create:async args=>{calls.push(args)}}};
 ...
 w.chrome.tabs.create=async()=>{throw Error('Unavailable')};w.document.querySelector('[data-support]').click();await new Promise(r=>setTimeout(r,0));assert.ok(w.document.querySelector('#saved').textContent);assert.equal(w.document.querySelector('[data-support]').dataset.opening,undefined);
 }finally{w.close()}
});
