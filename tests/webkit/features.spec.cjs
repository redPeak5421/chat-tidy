...
test('archive entry, native table multi-select, restore and delete confirmation',async({page,context},testInfo)=>{
 ...
 await page.goto('https://chatgpt.com/');await page.evaluate(()=>{globalThis.browser={storage:{local:{get:async d=>({...d,language:'en'}),set:async()=>{}},onChanged:{addListener(fn){globalThis.change=fn;}}},runtime:{id:'test',onMessage:{addListener(){}}}};globalThis.chrome=globalThis.browser;});
...
test('Bots preferences use the native section toggle and hide the complete group',async({page,context})=>{
 ...
 await page.goto('https://grok.com/');await page.evaluate(()=>{globalThis.browser={storage:{local:{get:async d=>d},onChanged:{addListener(fn){globalThis.change=fn;}}},runtime:{id:'test',onMessage:{addListener(){}}}};globalThis.chrome=globalThis.browser;const b=document.querySelector('#bots button');b.onclick=()=>{const expanded=b.getAttribute('aria-expanded')!=='true';b.setAttribute('aria-expanded',String(expanded));document.querySelector('#bots ul').hidden=!expanded;};});
 for(const f of ['core','batch','i18n','archive','bots','content'])await page.addScriptTag({path:'src/'+f+'.js'});await page.addStyleTag({path:'src/content.css'});
