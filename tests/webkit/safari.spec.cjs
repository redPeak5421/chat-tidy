...
async function fixture(context,page,{full=true}={}){
 ...
 await page.evaluate(()=>{globalThis.chrome={storage:{local:{get:async defaults=>({...defaults,enabled:true,language:'en'}),set:async()=>{}},onChanged:{addListener:()=>{}}},runtime:{id:'fixture',onMessage:{addListener:()=>{}}}};});
...
test('popup fits a compact Safari viewport and saves translated settings',async({page,context},info)=>{
 ...
 await page.addInitScript(()=>{globalThis.saved={};globalThis.chrome={storage:{local:{get:async d=>d,set:async d=>Object.assign(saved,d)}},tabs:{query:async()=>[]}};});
